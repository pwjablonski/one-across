...
export default createLogic({
  type: 'KEY_INPUTTED',
  async process({getState, action:{payload: {key}}}, dispatch, done) {
    ...
    switch(key) {
      ...
      case 'ArrowUp':
        if(currentDirection === 'across'){
          ...
          const {newClue, newHilightedCells} = findClueAndHilightedCells('down', currentCell, grid, gridnums);
          ...
          const {newClue, newHilightedCells} = findClueAndHilightedCells(currentDirection, newCurrentCell, grid, gridnums);
          dispatch(updateCurrentClue(newClue))
          dispatch(updateHilightedCells(newHilightedCells))
        }
        break;
      ...
      case 'ArrowLeft':
        if(currentDirection === 'down'){
          ...
          const {newClue, newHilightedCells} = findClueAndHilightedCells('across', currentCell, grid, gridnums);
          dispatch(updateCurrentClue(newClue))
          dispatch(updateHilightedCells(newHilightedCells))
        } else {
          const newCurrentCell = findNewCurrentCell(grid, currentCell, key)
      ...
      case 'Enter':
      case 'Tab': {
          const newCurrentCell = findNextClueStartingCell(currentCell, currentDirection, grid)
          dispatch(updateCurrentCell(newCurrentCell))
          const {newClue, newHilightedCells} = findClueAndHilightedCells(currentDirection, newCurrentCell, grid, gridnums);
          dispatch(updateCurrentClue(newClue))
          dispatch(updateHilightedCells(newHilightedCells))
